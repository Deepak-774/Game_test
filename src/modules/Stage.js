...
class Stage extends Container {
   ...
   */
  scaleToWindow() {
    const scaleX = window.innerWidth / MAX_X;
    const scaleY = window.innerHeight / MAX_Y;

    // Use a uniform scale to preserve the original aspect ratio
    const scale = Math.min(scaleX, scaleY);

    this.scale.set(scale, scale);

    // Center the stage within the renderer view
    const stageWidth = MAX_X * scale;
    const stageHeight = MAX_Y * scale;

    this.position.x = (window.innerWidth - stageWidth) / 2;
    this.position.y = (window.innerHeight - stageHeight) / 2;
  }

    ...
    $(document).ready(function documentReady() {
        ...
        Crafty.c("PlayerControls", {
            ...
            _enterframe: function () {
                ...
                var accelX = this._accel.x;

                if(IS_MOBILE) {
                    if(mobileInput.left && !mobileInput.right) {
                        accelX = -this.ACCEL;
                        this.flip();
                    } else if(mobileInput.right && !mobileInput.left) {
                        accelX = this.ACCEL;
                        this.unflip();
                    } else if(!mobileInput.left && !mobileInput.right) {
                        accelX = 0;
                    }
                }
        ...
        Crafty.scene("dead", function initDead() {
                    ...
                    if(++s > stars) {
                        ...
                        try {
                            if(window && window.parent) {
                                window.parent.postMessage({ type: "GAME_OVER", score: total }, "*");
                            }
                        } catch (err) {}
            ...
            // No restart interaction: game over screen is static and does not restart the game.
        });
        ...
        Crafty.scene("intro", function initIntro() {
            Crafty.background("url('assets/images/bg.png') repeat");

            var txt = Crafty.e("2D, DOM, Text").attr({
                x: 4,
                y: Crafty.viewport.height - 32,
                w: Crafty.viewport.width
            }).css({
                "font": "12px Verdana, Arial",
                "color": "#fff",
                "text-align": "center"
            }).text('Tap or press any key to start the game');

            Crafty.e("2D, Canvas, Image, Tween, Keyboard, Mouse")
            // .image(R.TITLE_PNG)

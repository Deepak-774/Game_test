...
$('.checkpoint1').css('left',((1-(track2+track3)/total)*100)+'%');$('.checkpoint2').css('left',((1-(track3)/total)*100)+'%');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.UP_ARROW,'jump');ig.input.bind(ig.KEY.SPACE,'jump');ig.input.bind(ig.KEY.ENTER,'ok');ig.input.bindTouch('#btn-left','left');ig.input.bindTouch('#btn-right','right');ig.input.bindTouch('#btn-start','ok');ig.music.add(sound1,'first');ig.music.add(sound2,'second');ig.music.add(sound3,'third');ig.music.loop=true;$('.win').fadeOut('slow');$('.death').fadeOut('slow');if(!soundEnabled){ig.music.volume=0;}
...
if(gameOver){ig.music.stop();this.gameOver=true;lowLag.play('death');$('.death').fadeIn(1000);$('.totalTime').html(Math.round(totalTime.delta())+' seconds');var deathPercent=(1-(-this.totalGameTimer.delta()/this.gameTotalTime))*100;$('.score').html(Math.round(deathPercent*100));setTimeout(function(){if(window.parent&&window.parent!==window&&window.game){window.parent.postMessage({type:'GAME_OVER',score:game.score},'*');}},1500);totalTime.pause();this.ekgs.push(this.spawnEntity(EntityFlatEKG,this.getRenderedWidth(),offsetTop));this.ekgs.push(this.spawnEntity(EntityFlatEKG,this.getRenderedWidth(),offsetTop));this.ekgs.push(this.spawnEntity(EntityFlatEKG,this.getRenderedWidth(),offsetTop));this.ekgs.push(this.spawnEntity(EntityFlatEKG,this.getRenderedWidth(),offsetTop));this.ekgs.push(this.spawnEntity(EntityFlatEKG,this.getRenderedWidth(),offsetTop));this.ekgs.push(this.spawnEntity(EntityFlatEKG,this.getRenderedWidth(),offsetTop));}
if(this.gameTimer.delta()>0||this.gameLevel==-1){this.gameLevel++;this.totalGameTimer=new ig.Timer();if(this.gameLevel==0){this.gameTimer.set(track1);this.player.setType('young');this.speed=250;this.heartbeat.probability=.4;ig.music.play('first');checkpoint=-1;this.totalGameTimer.set(this.gameTotalTime);}else if(this.gameLevel==1){this.player.setType('middle');this.gameTimer.set(track2);this.speed=260;this.heartbeat.probability=.6;ig.music.play('second');this.totalGameTimer.set(track2+track3);checkpoint=0;}else if(this.gameLevel==2){this.player.setType('old');this.gameTimer.set(track3);this.speed=300;this.heartbeat.probability=.7;ig.music.play('third');this.totalGameTimer.set(track3);checkpoint=1;}else{if(!this.winlocation){this.winlocation=this.screen.x+1;ig.music.stop();$('.win').fadeIn();$('.totalTime').html(Math.round(totalTime.delta())+' seconds');var winPercent=(1-(-this.totalGameTimer.delta()/this.gameTotalTime))*100;$('.score').html(Math.round(winPercent*100));setTimeout(function(){if(window.parent&&window.parent!==window&&window.game){window.parent.postMessage({type:'GAME_OVER',score:game.score},'*');}},1500);totalTime.pause();this.totalGameTimer=new ig.Timer();this.totalGameTimer.set(0);this.updateStatus();lowLag.play('win');}

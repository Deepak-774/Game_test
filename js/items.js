 ...
 * Now with direct drag functionality - no tap required!
 * 
 */
function buildObjClick(obj){
	if(obj.cursor != 'pointer'){
		...
		// Store drag state for this icon
		...
		obj.addEventListener("mousedown", function(evt) {
			if(!gamePause){
				obj.isDragging = true;
				obj.dragStartX = evt.stageX / scalePercent;
				obj.dragStartY = evt.stageY / scalePercent;
				obj.hasMoved = false;
			}
		});
		
		// Mouse/Touch move - detect movement and select if moved
		obj.addEventListener("pressmove", function(evt) {
			if(!gamePause && obj.isDragging){
				var currentX = evt.stageX / scalePercent;
				var currentY = evt.stageY / scalePercent;
				var deltaX = Math.abs(currentX - obj.dragStartX);
				var deltaY = Math.abs(currentY - obj.dragStartY);
				
				// If moved more than 10 pixels, select the item
				if(!obj.hasMoved && (deltaX > 10 || deltaY > 10)){
					obj.hasMoved = true;
					selectObj(obj);
				}
			}
		...
		// Mouse/Touch up - check if dragged back to original position (cancel)
		obj.addEventListener("pressup", function(evt) {
			if(!gamePause && obj.isDragging){
				obj.isDragging = false;
				
				// If item was selected and dragged
				if(obj.hasMoved && curItem && curItem.name === obj.name){
					var currentX = evt.stageX / scalePercent;
					var currentY = evt.stageY / scalePercent;
					
					// Check if released near original position (within 80 pixels)
					var distX = Math.abs(currentX - obj.oriX);
					var distY = Math.abs(currentY - obj.oriY);
					var distance = Math.sqrt(distX * distX + distY * distY);
					
					if(distance < 80){
						// Dragged back to shelf - cancel/restore
						console.log('Item dragged back to shelf - canceling');
						restoreObject(curItem);
					}
				}

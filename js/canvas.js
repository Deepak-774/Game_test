...
function initGameCanvas(w,h){
	...
	// MOBILE WEBVIEW NUCLEAR OPTION - Document-level touch prevention
	setupMobileTouchPrevention();
}

/*!
 * 
 * MOBILE TOUCH PREVENTION - Nuclear option for webview compatibility
 * 
 */
function setupMobileTouchPrevention() {
	// Force canvas touch properties for drag interactions
	...
	// Global document touch prevention with selective allowance for game interactions
	document.addEventListener('touchstart', function(e) {
		var target = e.target;
		var isCanvas = target.tagName === 'CANVAS' || target.id === 'gameCanvas';
		var isButton = target.tagName === 'BUTTON' || target.closest('button');
		var isGameArea = target.closest('#canvasHolder') || target.closest('#mainHolder');
		
		// Allow touch on canvas, buttons, and game areas
		if (!isCanvas && !isButton && !isGameArea) {
			e.preventDefault();
			e.stopPropagation();
		}
		// Don't prevent default on canvas to allow CreateJS touch handling
	...
	document.addEventListener('touchmove', function(e) {
		...
		// Allow touchmove on canvas for dragging, prevent everywhere else to stop scrolling
		if (!isCanvas && !isGameArea) {
			e.preventDefault();
			e.stopPropagation();
		}
	}, { passive: false, capture: true });
	...
	document.addEventListener('touchend', function(e) {
		...
		// Allow touch end on canvas, buttons, and game areas
		if (!isCanvas && !isButton && !isGameArea) {
			e.preventDefault();
			e.stopPropagation();
		}
	...
	document.addEventListener('pointerdown', function(e) {
		var target = e.target;
		var isCanvas = target.tagName === 'CANVAS' || target.id === 'gameCanvas';
		var isAllowedElement = target.tagName === 'BUTTON' || target.closest('button') || isCanvas;
	...
	document.addEventListener('pointermove', function(e) {
		// Allow pointer move on canvas for game interaction
		var target = e.target;
		var isCanvas = target.tagName === 'CANVAS' || target.id === 'gameCanvas';

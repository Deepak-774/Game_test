...
var ScreenGame =
{
    ...
    UpdateCanvasSize: function(width, height)
    {
        ...
        {
            // Landscape mode
            h = Math.min(Main.HEIGHT_MAX, height / Main.SCALE);
            scale = (h - Main.HEIGHT_MIN) / (Main.HEIGHT_MAX - Main.HEIGHT_MIN);
            this.s_container.scaleX = this.s_container.scaleY = Math.max(Main.SCALE, height / 640);
            // Reset position for landscape
            this.s_container.x = 0;
            this.s_container.y = 0;
        }
        else
        {
            // Portrait mode - center game content
            h = Math.min(Main.HEIGHT_MAX, height / Main.SCALE);
            scale = (h - Main.HEIGHT_MIN) / (Main.HEIGHT_MAX - Main.HEIGHT_MIN);
            
            // Scale to fit screen properly
            var gameScale = Math.min(width / 1136, height / 640);
            this.s_container.scaleX = this.s_container.scaleY = gameScale;
            
            // Center both horizontally and vertically
            this.s_container.x = (width - 1136 * gameScale) / 2;
            this.s_container.y = (height - 640 * gameScale) / 2;
        }

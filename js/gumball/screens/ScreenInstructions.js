...
var ScreenInstructions =
{
    ...
    Init: function(inGame)
    {
        ...
        this.s_countdownText = new createjs.Text();
        this.s_countdownText.textAlign = "left";
        this.s_countdownText.color = "#ffffff";
        this.s_countdownText.font = getFontStyle(".close");
        this.s_countdownText.text = "";
        this.s_countdownText.visible = false;
        ...
        //
        this.s_container.addChild(this.s_bg, this.s_imagesContainer, this.s_copy, this.s_countdownText, this.s_btnNext, this.s_btnPrev);
    ...
    Remove: function()
    {
        ...
        if (this.s_countdownTimer) {
            clearInterval(this.s_countdownTimer);
            this.s_countdownTimer = null;
        }
    ...
    StartCountdown: function()
    {
        ...
        this.s_countdown = 2;
        this.s_countdownText.visible = true;
        this.s_countdownText.text = "Next in: " + this.s_countdown;
        
        this.s_countdownTimer = setInterval(function() {
            me.s_countdown--;
            if (me.s_countdown > 0) {
                me.s_countdownText.text = "Next in: " + me.s_countdown;
            } else {
                me.s_countdownText.visible = false;
                clearInterval(me.s_countdownTimer);
                me.s_countdownTimer = null;
    ...
    UpdateCanvasSize: function(width, height)
    {
        ...
        else
        {
            ...
            s = Math.max(Main.SCALE * 0.4, Math.min(width / 1000, height / 1200));
            
            // Instruction images - smaller, centered like game screen
            bb = this.s_imagesContainer.getBounds();
            var imageScale = Math.min(width / bb.width * 0.7, height / bb.height * 0.4);
            this.s_imagesContainer.scaleX = this.s_imagesContainer.scaleY = imageScale;
            this.s_imagesContainer.x = (width - bb.width * imageScale) >> 1;
            this.s_imagesContainer.y = 0.15 * height;

            // Countdown text - much smaller in top left
            var countdownScale = Math.max(s * 0.6, width / 1000);
            this.s_countdownText.scaleX = this.s_countdownText.scaleY = countdownScale;
            this.s_countdownText.x = 15 * countdownScale;
            this.s_countdownText.y = 30 * countdownScale;
            FixText(this.s_countdownText);

            // Instruction text - much smaller
            var textScale = Math.max(s * 0.7, width / 800);
            this.s_copy.scaleX = this.s_copy.scaleY = textScale;
            this.s_copy.x = width >> 1;
            this.s_copy.y = 0.75 * height;
            FixText(this.s_copy);

            // Navigation buttons - much smaller
            bb = this.s_btnNext.getBounds();
            var btnScale = Math.max(s * 0.6, width / 600);
            this.s_btnNext.scaleX = this.s_btnNext.scaleY = btnScale;
            this.s_btnNext.x = width - (10 + bb.width) * btnScale;
            this.s_btnNext.y = height - (10 + bb.height) * btnScale;

            bb = this.s_btnPrev.getBounds();
            this.s_btnPrev.scaleX = this.s_btnPrev.scaleY = btnScale;
            this.s_btnPrev.x = 10 * btnScale;
            this.s_btnPrev.y = height - (10 + bb.height) * btnScale;

            if (this.s_btnClose)
            {
                bb = this.s_btnClose.getBounds();
                this.s_btnClose.scaleX = this.s_btnClose.scaleY = btnScale;
                this.s_btnClose.x = width - (15 + bb.width) * btnScale;
                this.s_btnClose.y = 15 * btnScale;
            }
        }

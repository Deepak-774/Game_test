...
{
	// ignore clicks starting on the help button in the HUD
	var osd = document.getElementById('osdContainer');
	if( osd && event.target ){
		var node = event.target;
		while(node && node !== osd){
			if( node.className && typeof node.className === 'string' && node.className.indexOf('helpButton') !== -1 ){
				return; // let normal click handler run, no joystick
			}
...
VirtualJoystick.prototype._onTouchStart	= function(event)
{
	...
	// ignore touches starting on the help button in the HUD
	var osd = document.getElementById('osdContainer');
	if( osd && event.target ){
		var node = event.target;
		while(node && node !== osd){
